...
#define define_show_state_freeze_ull_function(_name) \
static ssize_t show_state_freeze_##_name(struct cpuidle_state *state, \
					 struct cpuidle_state_usage *state_usage, \
					 char *buf)				\
{ \
	return sprintf(buf, "%llu\n", state_usage->freeze_##_name);\
}

define_show_state_freeze_ull_function(usage);
define_show_state_freeze_ull_function(time);

#define define_one_state_freeze_ro(_name, show) \
static struct cpuidle_state_attr attr_freeze_##_name = \
	__ATTR(_name, 0444, show, NULL)

define_one_state_freeze_ro(usage, show_state_freeze_usage);
define_one_state_freeze_ro(time, show_state_freeze_time);

static struct attribute *cpuidle_state_freeze_attrs[] = {
	&attr_freeze_usage.attr,
	&attr_freeze_time.attr,
	NULL
};

static const struct attribute_group cpuidle_state_freeze_group = {
	.name	= "freeze",
	.attrs	= cpuidle_state_freeze_attrs,
};

static void cpuidle_add_freeze_attr_group(struct cpuidle_state_kobj *kobj)
{
	int ret;
	...
	ret = sysfs_create_group(&kobj->kobj, &cpuidle_state_freeze_group);
	if (ret)
		pr_debug("%s: sysfs attribute group not created\n", __func__);
}

static void cpuidle_remove_freeze_attr_group(struct cpuidle_state_kobj *kobj)
{
	if (kobj->state->enter_freeze)
		sysfs_remove_group(&kobj->kobj, &cpuidle_state_freeze_group);
}
#else
static inline void cpuidle_add_freeze_attr_group(struct cpuidle_state_kobj *kobj) { }
static inline void cpuidle_remove_freeze_attr_group(struct cpuidle_state_kobj *kobj) { }
...
{
	cpuidle_remove_freeze_attr_group(device->kobjs[i]);
...
static int cpuidle_add_state_sysfs(struct cpuidle_device *device)
{
	...
	for (i = 0; i < drv->state_count; i++) {
		...
		cpuidle_add_freeze_attr_group(kobj);
